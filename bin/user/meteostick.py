            ...
            if n >= 5:
                ...
                if parts[0] == 'W':
                    ...
                    if th1_ch != 0 and data['channel'] == th1_ch:
                        ...
                        logerr("[ 1 ] Temp %s Hum %s %s" % (float(parts[2]), float(parts[3])))
                    elif th2_ch != 0 and data['channel'] == th2_ch:
                        data['bat_th_2'] = bat
                        data['temp_2'] = float(parts[2]) # C
                        data['humid_2'] = float(parts[3]) # %
                        logerr("[ 2 ] Temp %s Hum %s %s" % (float(parts[2]), float(parts[3])))
                    else:
                        data['bat_iss'] = bat
                        data['temperature'] = float(parts[2]) # C
                        data['humidity'] = float(parts[3]) # %
                        logerr("[def] Temp %s Hum %s %s" % (float(parts[2]), float(parts[3])))
            else:
